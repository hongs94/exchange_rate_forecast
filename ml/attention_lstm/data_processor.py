...
class DataProcessor:
    def __init__(self):
        ...
        self.origin_data["dgs10_jpy10_diff"] = (self.origin_data["dgs10"] - self.origin_data["jpy10"])
        self.origin_data["dgs10_eur10_diff"] = (self.origin_data["dgs10"] - self.origin_data["eur10"])
    ...
    def get_proceed_data(self, target) -> pd.DataFrame:
        ...
        if not features_for_target:
            raise ValueError(f"'{target}'에 대한 Feature 목록 정의 안됨.")
    ...
    def get_sequence_data(self, target: str):
        ...
        # 시퀀스 생성
        X_seq, y_seq, y_idxs = self.create_sequences(X=features_scaled, y=target_scaled, seq_len=LOOK_BACK, y_index=y.index)
