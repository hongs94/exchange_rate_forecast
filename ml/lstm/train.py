...
def train():
    ...
    for target in targets:
        ...
        for i, (train_idx, test_idx) in enumerate(rolling_split_index(total_len)):
            ...
            if i == 0:
                print("최적 하이퍼파라미터 탐색")
                best_params = find_best_hyperparams(X_train, y_train, X_test, y_test, X_seq.shape[2])
                print(f"최적 하이퍼파라미터: LSTM Units={best_params[0]}, Dropout={best_params[1]}, LR={best_params[2]}")
